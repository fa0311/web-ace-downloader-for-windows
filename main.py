...
import web_ace_downloader.web_ace_downloader as web_ace_downloader
import webbrowser

APP_NAME = "web-ace-downloader-for-windows"
VERSION = "1.0.0"
...
class input_window:
    def __init__(self, debug=False):
        ...
        self.window.title("web-ace-downloader-for-windows")
    ...
    def run(self):
        ...
        tkinter.Label(
            self.window,
            wraplength=380,
            text="このツールは不正ダウンロードを助長するものではありません。利用が終わった際は速やかにダウンロードした著作物を記録媒体から削除して下さい。ダウンロードした著作物をアーカイブしたり第三者に公開、アップロードする行為は著作権侵害になる場合があります。",
        ).grid(
            column=0,
            row=300,
            columnspan=2,
    ...
    def update_dialog(self, event=None):
        webbrowser.open(
            "https://github.com/fa0311/web-ace-downloader-for-windows/releases"
        )
    ...
    def download(self):
        ...
        if not str.isdigit(self.wait_box.get()):
            self.wait_box.set("0")
        progress_window().run().jpd_run(dir=dir).download(
            url,
...
class progress_window:
    ...
    def jpd_run(self, dir="./"):
        self.jpd = web_ace_downloader.web_ace_downloader(dir=dir)
        return self
